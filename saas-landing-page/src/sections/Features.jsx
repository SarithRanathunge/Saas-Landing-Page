                ...
                <div className="realtive flex md:flex-wrap flex-nowrap border-2 border-s3 rounded-7xl md:overflow-hidden max-md:flex-col feature-after md:g7 max-md:border-none max-md:rounded-none max-md:gap-3">
                    { features.map(({id, icon, caption, title, text, button }) => (
                        <div key={id} className="relative z-2 md:px-10 px-5 md:pb-10 pb-5 flex-50 max-md:g7 max-md:border-2 max-md:border-s3 max-md:rounded-3xl max-md:flex-320">
                            ...
                            <h3 className="mb-11 body-1 max-md:mb-8 max-md:body-3">{text}</h3>
                            <Button icon={button.icon}>{button.title}</Button>
                        </div>
                    ))}

                    <ul className="realtive flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
                        <div className="absolute bg-s3/20 top-[38%] left-0 right-0 w-full h-[1px] z-10"/>
                        {details.map(({id, icon, title})=>(
                               <li key={id} className="realtive pt-16 px-4 pb-14">
                                    ...
                                    <h3 className="realtive z-2 max-w-36 mx-auto my-0 base-small text-center uppercase">
                                        {title}
                                    </h3>
                               </li>
                        ))}
